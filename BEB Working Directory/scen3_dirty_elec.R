...
scen3_emissions_model <- function(assumed_bus_life = 14, 
  ...
  year_replace <- 2023:(2023 + length(number_replace))
  ...
  n <- 13
  egrid_dirty <- do.call("rbind", replicate(n, egrid_running_total[1:27,], simplify = FALSE))
  bus_data <- left_join(bus_data, egrid_dirty, by = c("eGRID.Subregion..1" = "eGRID.subregion.acronym",
  ...
  bus_data[, "fuel.wtt..VOC..mt"] <- bus_data[,"VOC..g.gal"] /
    bus_data[,"bus.mpg"] * 
    bus_data[, "Total.Miles.on.Active.Vehicles.During.Period"] *
    (bus_data$Year.Start - 2023) / 1000000
