...
# Scenario 3: 5% Yearly Transition

scen3_emissions_model <- function(assumed_bus_life = 14, 
  ...
  bus_data <- bus_mileage
  
  # Find out how many buses is 5% of the fleet
  five_percent <- (sum(bus_data$Total.Fleet.Vehicles)*0.05) %>% round()
  # Order the data by phase out year
  bus_data <- bus_data[order(bus_data$Final.Year),]  
  # Initialize a field that will serve as a running sum of bus counts
  bus_data$total.veh.running.sum <- bus_data$Total.Fleet.Vehicles
  print("Checkpoint 1")
  # The first row's running total is just itself so we can start on row two
  i <- 2
  while (i <= nrow(bus_data)) {
    
    # Calculate the running total
    bus_data[i, "total.veh.running.sum"] <- bus_data[i-1, "total.veh.running.sum"] + bus_data[i, "total.veh.running.sum"]
    
    # Progress Monitor
    if (i %% 100 == 0){
      print(paste0(i/100, "/", round(nrow(bus_data)/100, digits = 0)))
    }
  ...
  number_replace <- seq(0, 
                        sum(bus_data$Total.Fleet.Vehicles) + 
                          five_percent, 
                        five_percent)
  # Create the vector of years the replacements will span
  year_replace <- 2021:(2021 + length(number_replace))
  
  # Initialize a column that assigns each fleet a replacement year
  bus_data$Year.Start <- 0
  
  for (i in 1:length(number_replace)) {
    
    j <- i + 1
    
    for (k in 1:nrow(bus_data)) {
      
      if ((number_replace[i] <= bus_data[k, "total.veh.running.sum"]) && 
          (bus_data[k, "total.veh.running.sum"] <= number_replace[j])){
        
        # If a row's running total falls between a year_replace bounds
        # then assign that portion of the fleet to that year 
        bus_data[k, "Year.Start"] <- year_replace[i]
        
      }
    }
    print(paste0(i, "/", length(number_replace)))
  }
  # Does not allow for splitting the agency's fleets so the 5% is a rough calculation
  
  # The model is for an all-electric fleet
  bus_data$beb.mpg <- bus_data$bus.mpg
  bus_data[bus_data$Fuel.Type != "Electric Battery", "beb.mpg"] <- elec.mpkwh.replacement
  # Establish a conventional fuel type and an electric fuel type
  bus_data$cFuel.Type <- bus_data$Fuel.Type
  bus_data$Fuel.Type <- "Electric Battery"
  
  # Emissions from Electricity Generation
  # "egrid_running_total" calculates how many grams/kWh/lifetime the BEBs generate
  # Therefore we can remove "year" value from calculations
  
  bus_data <- left_join(bus_data, egrid_zip[,c(2,4)], by = c("Zip.Code" = "ZIP..numeric."))
  ...
  if (largest_pack == FALSE){
    bus_data$batt_size <- bus_data$consumption_used * bus_data$Average.Daily.Miles.on.Active.Vehicles.During.Period / DOD
    if(sum(bus_data$batt_size > bus_data$batt_high) > 0){
      bus_data[bus_data$batt_size > bus_data$batt_high, "batt_size"] <- bus_data[bus_data$batt_size > bus_data$batt_high, "batt_high"]
    } else if (sum(bus_data$batt_size < bus_data$batt_low) > 0){
      bus_data[bus_data$batt_size < bus_data$batt_low, "batt_size"] <- bus_data[bus_data$batt_size < bus_data$low, "batt_low"]
    }
  } else {
    bus_data$batt_size <- bus_data$batt_high
  }
  ...
  for (chem in batt_chem_list) {
    ...
    bus_data[bus_data$Fuel.Type == "Electric Battery" & bus_data$batt_chem == chem, "batt_VOC..mt"] <- ((batt_lc_emissions[batt_lc_emissions$Total.grams.per.lifetime.per.kWh == "VOC", chem] * 
                                                                                                           ...
                                                                                                           bus_data[bus_data$batt_chem == chem, "Total.Fleet.Vehicles"] * 
                                                                                                           (1 + batt_replace))/ 1000000)
  ...
  bus_data[, "fuel.wtt..VOC..mt"] <- bus_data[,"VOC..g.gal"] /
    bus_data[,"bus.mpg"] * 
    bus_data[, "Total.Miles.on.Active.Vehicles.During.Period"] *
    (bus_data$Year.Start - 2020) / 1000000
  ...
  # Then, tank to wheel:
  bus_data <- left_join(bus_data, emfac, by = c("cFuel.Type" = "Fuel", "Manufacture.Year" = "Model.Year"))
  
  bus_data$fuel.ttw..NOx_RUNEX..mt <- bus_data$NOx_RUNEX..g.gal / bus_data$bus.mpg * bus_data$Total.Miles.on.Active.Vehicles.During.Period * (bus_data$Year.Start - 2023) / 1000000
  bus_data$fuel.ttw..CO2e_RUNEX..mt <- bus_data$CO2e_RUNEX..g.gal / bus_data$bus.mpg * bus_data$Total.Miles.on.Active.Vehicles.During.Period * (bus_data$Year.Start - 2023) / 1000000
  bus_data$fuel.ttw..CO_RUNEX..mt <- bus_data$CO_RUNEX..g.gal / bus_data$bus.mpg * bus_data$Total.Miles.on.Active.Vehicles.During.Period * (bus_data$Year.Start - 2023) / 1000000
  bus_data$fuel.ttw..PM10_RUNEX..mt <- bus_data$PM10_RUNEX..g.gal / bus_data$bus.mpg * bus_data$Total.Miles.on.Active.Vehicles.During.Period * (bus_data$Year.Start - 2023) / 1000000
  bus_data$fuel.ttw..PM2.5_RUNEX..mt <- bus_data$PM2.5_RUNEX..g.gal / bus_data$bus.mpg * bus_data$Total.Miles.on.Active.Vehicles.During.Period * (bus_data$Year.Start - 2023) / 1000000
  bus_data$fuel.ttw..VOC_RUNEX..mt <- bus_data$VOC_RUNEX..g.gal / bus_data$bus.mpg * bus_data$Total.Miles.on.Active.Vehicles.During.Period * (bus_data$Year.Start - 2023) / 1000000
  bus_data$fuel.ttw..SOx_RUNEX..mt <- bus_data$SOx_RUNEX..g.gal / bus_data$bus.mpg * bus_data$Total.Miles.on.Active.Vehicles.During.Period * (bus_data$Year.Start - 2023) / 1000000
  
  bus_data$fuel.ttw..PM10_PMBW..mt <- bus_data$PM10_PMBW..g.mi * bus_data$Total.Miles.on.Active.Vehicles.During.Period * (bus_data$Year.Start - 2023) / 1000000
  bus_data$fuel.ttw..PM2.5_PMBW..mt <- bus_data$PM2.5_PMBW..g.mi * bus_data$Total.Miles.on.Active.Vehicles.During.Period * (bus_data$Year.Start - 2023) / 1000000
  bus_data$fuel.ttw..PM10_PMTW..mt <- bus_data$PM10_PMTW..g.mi * bus_data$Total.Miles.on.Active.Vehicles.During.Period * (bus_data$Year.Start - 2023) / 1000000
  bus_data$fuel.ttw..PM2.5_PMTW..mt <- bus_data$PM2.5_PMTW..g.mi * bus_data$Total.Miles.on.Active.Vehicles.During.Period * (bus_data$Year.Start - 2023) / 1000000
  ...
  # Buses not set to transition in the next 14 years will follow the base case path
  ...
  # Merge the buses that were transitioned to those that were not
  transitioned_buses <- final_data[final_data$Final.Year <= 2035,]
  non_transitioned <- base_case[base_case$Final.Year > 2035,]
  
  #Final results
  final_data <- rbind.data.frame(transitioned_buses,non_transitioned)
