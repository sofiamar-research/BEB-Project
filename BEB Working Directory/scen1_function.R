...
# Scenario 1: Immediate flip of all conventional vehicles to BEB

scen1_emissions_model <- function(assumed_bus_life = 14, 
  ...
  print(summary(bus_data))
  
  for (chem in batt_chem_list) {
    
    # For each chemistry, compute each emission type
    bus_data[bus_data$batt_chem == chem, "batt_VOC..mt"] <- ((batt_lc_emissions[batt_lc_emissions$Total.grams.per.lifetime.per.kWh == "VOC", chem] * 
                                                                bus_data[bus_data$batt_chem == chem,"batt_size"] * bus_data[bus_data$batt_chem == chem, "Total.Fleet.Vehicles"] * 
                                                                (1 + batt_replace))/ 1000000)
    bus_data[bus_data$batt_chem == chem, "batt_CO..mt"] <- ((batt_lc_emissions[batt_lc_emissions$Total.grams.per.lifetime.per.kWh == "CO", chem] * 
                                                               bus_data[bus_data$batt_chem == chem,"batt_size"] * bus_data[bus_data$batt_chem == chem, "Total.Fleet.Vehicles"] * 
                                                               (1 + batt_replace))/ 1000000)
    bus_data[bus_data$batt_chem == chem, "batt_NOx..mt"] <- ((batt_lc_emissions[batt_lc_emissions$Total.grams.per.lifetime.per.kWh == "NOx", chem] * 
                                                                bus_data[bus_data$batt_chem == chem,"batt_size"] * bus_data[bus_data$batt_chem == chem, "Total.Fleet.Vehicles"] * 
                                                                (1 + batt_replace))/ 1000000)
    bus_data[bus_data$batt_chem == chem, "batt_PM10..mt"] <- ((batt_lc_emissions[batt_lc_emissions$Total.grams.per.lifetime.per.kWh == "PM10", chem] * 
                                                                 bus_data[bus_data$batt_chem == chem,"batt_size"] * bus_data[bus_data$batt_chem == chem, "Total.Fleet.Vehicles"] * 
                                                                 (1 + batt_replace))/ 1000000)
    bus_data[bus_data$batt_chem == chem, "batt_PM2.5..mt"] <- ((batt_lc_emissions[batt_lc_emissions$Total.grams.per.lifetime.per.kWh == "PM2.5", chem] * 
                                                                  bus_data[bus_data$batt_chem == chem,"batt_size"] * bus_data[bus_data$batt_chem == chem, "Total.Fleet.Vehicles"] * 
                                                                  (1 + batt_replace))/ 1000000)
    bus_data[bus_data$batt_chem == chem, "batt_SOx..mt"] <- ((batt_lc_emissions[batt_lc_emissions$Total.grams.per.lifetime.per.kWh == "SOx", chem] * 
                                                                bus_data[bus_data$batt_chem == chem,"batt_size"] * bus_data[bus_data$batt_chem == chem, "Total.Fleet.Vehicles"] * 
                                                                (1 + batt_replace))/ 1000000)
    bus_data[bus_data$batt_chem == chem, "batt_BC..mt"] <- ((batt_lc_emissions[batt_lc_emissions$Total.grams.per.lifetime.per.kWh == "BC", chem] * 
                                                               bus_data[bus_data$batt_chem == chem,"batt_size"] * bus_data[bus_data$batt_chem == chem, "Total.Fleet.Vehicles"] * 
                                                               (1 + batt_replace))/ 1000000)
    bus_data[bus_data$batt_chem == chem, "batt_OC..mt"] <- ((batt_lc_emissions[batt_lc_emissions$Total.grams.per.lifetime.per.kWh == "OC", chem] * 
                                                               bus_data[bus_data$batt_chem == chem,"batt_size"] * bus_data[bus_data$batt_chem == chem, "Total.Fleet.Vehicles"] * 
                                                               (1 + batt_replace))/ 1000000)
    bus_data[bus_data$batt_chem == chem, "batt_CO2e..mt"] <- ((batt_lc_emissions[batt_lc_emissions$Total.grams.per.lifetime.per.kWh == "GHGs", chem] * 
                                                                 bus_data[bus_data$batt_chem == chem,"batt_size"] * bus_data[bus_data$batt_chem == chem, "Total.Fleet.Vehicles"] * 
                                                                 (1 + batt_replace))/ 1000000)
  ...
  # First well to tank:
  # eGRID is the electric battery equivalent to wtt therefore
  # No other fuel gets evaluated for well to tank
  ...
  bus_data <- left_join(bus_data, emfac, by = c("Fuel.Type" = "Fuel", 
                                                "Manufacture.Year" = "Model.Year"))
  
  # No "Running Exhaust" from Battery Electric Buses
  # These should all result in zero values
