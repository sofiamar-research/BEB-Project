...
# Emfac comes with units of g/mi, this provides the mpg used in those calculations so we may apply our own mileage
...
manufacture_year <- ggplot(data = data_sum, aes(x=Manufacture.Year, y=Active.Fleet.Vehicles)) + 
  ...
  labs(x = "Manufacture Year", y = str_wrap("Number of Active U.S. Transit Buses in 2021", 25))
...
bean_pred <- data.frame(year = 2021:2040)
b.value <- 0.858
...
beb_frame <- beb_prices$Mean - (batt_specs$batt_mean %>% mean())*0.7 * bean_pred[bean_pred$year == 2021, "Medium"]
...
# Adjust emfac from g/mi to g/gal with agency-avg mpg
# Analysis calculations will recalculate g/mi with bus-specific mpg
...
emfac_summary <- emfac %>% group_by(Fuel) %>% summarize(mean(CO_RUNEX..g.gal))
...
egrid_simp <- egrid[, c(1, 13, 9, 8)]

# Proportion of emission reduction from 2021 to 2035
elec_reduction <- data.frame(scen_type = c("Min", "Avg", "Max"),
                             NOx = c(0.42, 0.62, 0.82),
                             SO2 = c(0.49, 0.73, 0.94),
                             CO2 = c(0.5, 0.65, 0.79))
...
for (i in c(1:3)) {
  ...
  while (year <= 2035){
    ...
    egrid_decrease <- egrid_simp[1:4]
    
    egrid_decrease$year <- year
    
    if(i == 1){
      egrid_decrease$sensitivity <- "min"
    } 
    if(i == 2){
      egrid_decrease$sensitivity <- "avg"
    } 
    else if (i == 3){
      egrid_decrease$sensitivity <- "max"
    }
    
    egrid_decrease[, 2:4] <- egrid_simp[, 2:4] - egrid_simp[, 2:4] * t(elec_reduction[i, 2:4]*((year - 2021)/14))
    
    if(year == 2021 & i == 1){
      
      egrid_running_total <- egrid_decrease
      
    } else {
      egrid_running_total <- rbind.data.frame(egrid_running_total, egrid_decrease)
    }
...
ggplot(data = plot_data, aes(x = year, y = eGRID.subregion.annual.CO2e.non.baseload.output.emission.rate..g.kWh)) +
  geom_line(aes(color = sensitivity))+
  theme_bw() +
  scale_x_continuous(breaks = 2021:2035) +
...
for (i in 2021:2035){
  ...
  egrid_sample <- filter(egrid_running_total, year>=i)
  
  print(unique(egrid_sample$year))
  
  total_decrease <- egrid_sample %>%  group_by(eGRID.subregion.acronym, sensitivity) %>%
  ...
  total_decrease$year.start <- i
  
  if(i == 2021){
    
    decrease_egrid <- total_decrease
    
  } else {
   
    decrease_egrid <- rbind.data.frame(decrease_egrid, total_decrease)
     
  }
  
}

## eGRID EMISSIONS ARE SUMMED FROM 2021 UNTIL 2035

egrid_emissions <- left_join(decrease_egrid, egrid_zip[,c(2,4)], 
                             by = c ("eGRID.subregion.acronym" = "eGRID.Subregion..1"))

plot_data <- decrease_egrid[decrease_egrid$eGRID.subregion.acronym == "SRVC",]
...
print(paste0("Roughly ", round((y1/x1 - 1)*100, 2),"% of the Active fleet has seating capacity greater than 60"))
print(paste0("Roughly ", round((y2/x2 - 1)*100, 2),"% of the Total fleet has seating capacity greater than 60"))
...
scen_0 <- base_emissions_model(assumed_bus_life = 14, 
...
scen_1 <- scen1_emissions_model(assumed_bus_life = 14, 
                                elec_eff_total = (elec_transm_dist_eff * plug_eff),
                                consumption_proportion = 0.5,
                                batt_replace = 1,
                                largest_pack = FALSE,
                                DOD = 0.7,
                                egrid_reduction = "Average")
scen_2 <- scen2_emissions_model(assumed_bus_life = 14, 
                                elec_eff_total = (elec_transm_dist_eff * plug_eff),
                                consumption_proportion = 0.5,
                                batt_replace = 1,
                                largest_pack = FALSE,
                                DOD = 0.7,
                                egrid_reduction = "Average")
scen_3 <- scen3_emissions_model(assumed_bus_life = 14, 
                                elec_eff_total = (elec_transm_dist_eff * plug_eff),
                                consumption_proportion = 0.5,
                                batt_replace = 1,
                                largest_pack = FALSE,
                                DOD = 0.7,
                                egrid_reduction = "Average")
scen_4 <- scen4_emissions_model(assumed_bus_life = 14, 
                                elec_eff_total = (elec_transm_dist_eff * plug_eff),
                                consumption_proportion = 0.5,
                                batt_replace = 1,
                                largest_pack = FALSE,
...
yearly_plot <- total_scen_melt %>% group_by(Year.Start,
                                            Scenario,
                                            Emission.Type) %>%
  summarise(Emissions.mt = sum(`Emissions in mt`, na.rm = TRUE))
...
total_low_scen$Final.Year <- 2020
...
yearly_low_plot <- total_low_scen_melt %>% group_by(Year.Start,
                                                      Scenario,
                                                      Emission.Type) %>%
  summarise(Emissions.mt.low = sum(`Emissions in mt`, na.rm = TRUE))
...
total_high_scen$Final.Year <- 2020
...
yearly_high_plot <- total_high_scen_melt %>% group_by(Year.Start,
                                                      Scenario,
                                                      Emission.Type) %>%
  summarise(Emissions.mt.high = sum(`Emissions in mt`, na.rm = TRUE))
...
emission_reduction <- left_join(yearly_plot, 
                                yearly_low_plot, 
                                by = c("Year.Start",
                                       "Scenario", 
                                       "Emission.Type"))
emission_reduction <- left_join(emission_reduction, 
                                yearly_high_plot, 
                                by = c("Year.Start", 
                                       "Scenario", 
                                       "Emission.Type"))

base_emissions <- emission_reduction[emission_reduction$Scenario == 0,]
base_emissions[,4:6] <- base_emissions[,4:6]
emission_reduction <- left_join(emission_reduction, 
                                base_emissions,
                                by = c("Emission.Type",
                                       "Year.Start"))

emission_reduction$avg.reduction <- round((emission_reduction$Emissions.mt.y - 
                                             emission_reduction$Emissions.mt.x)/1000000
                                          #emission_reduction$Emissions.mt.y)*100
                                          , 5)
emission_reduction$min.reduction <- round((emission_reduction$Emissions.mt.high.y - 
                                             emission_reduction$Emissions.mt.high.x)/1000000
                                          #emission_reduction$Emissions.mt.high.y)*100
                                          , 5)
emission_reduction$max.reduction <- round((emission_reduction$Emissions.mt.low.y - 
                                             emission_reduction$Emissions.mt.low.x)/1000000
                                          #emission_reduction$Emissions.mt.low.y)*100
                                          , 5)

data <- emission_reduction[emission_reduction$Scenario.x != 0 & 
                             emission_reduction$Scenario.x != 1 & 
                             emission_reduction$Emission.Type != "OC" &
                             emission_reduction$Emission.Type != "BC"
                           ,c(1:3, 11:13)]

data$Scenario.x <- factor(data$Scenario.x)

# emiss_ylim <- data.frame(Emissions = c("CO", "CO2e", "NOx", "PM10", "PM2.5", "SOx", "VOC"),
#                          ymin = c(80, 80, 80, 80, 80, 70, 80),
#                          ymax = c(100, 100, 100, 100, 100, 100, 100))

for (emission in unique(data$Emission.Type)) {
  
  temp <- ggplot(data = data[data$Emission.Type == emission,], 
                 aes(Year.Start, 
                     avg.reduction, 
                     group=Scenario.x,
                     color=Scenario.x)) +
    ...
    scale_x_continuous(limits = c(2021,2035),
      breaks = seq(2021,2035,1)) +
    geom_ribbon(aes(ymin=min.reduction, 
                    ymax=max.reduction, 
                    group=Scenario.x, 
                    fill=Scenario.x),
                size = 0.1,
    ...
    theme_classic() +
    labs(x = "Year",
         y = str_wrap(paste0(emission, " Reduction from base case scenario (MMT)"), 30),
         color="Scenario",
         fill = "Scenario")
  ...
  if (emission != "CO2e"){
    ...
    temp <-temp +
      geom_text(x=2022.5, 
                y=-0.5, 
                label="Increased Emissions from Base Case", 
                color="grey15", 
                show.legend = FALSE, 
                size=2) +
      geom_text(x=2022.5, 
                y=0.5, 
                label="Decreased Emissions from Base Case", 
                color="grey15", 
                show.legend = FALSE, 
                size=2)
  ...
  assign(paste0(emission, "_reduc"), temp)
...
#### Graphical Abstract ####
scen_labels_ <- c("Base Case (Fossil-Fueled Buses)", 
                 "Replace All Buses Now", 
                 "Natural Phase Out", 
                 "Replace 5% Fleet Yearly", 
                 "Replace Top 100 Fleets") %>% str_wrap(width = 6)
...
ghgs <- ggplot(data = ghg_plot, aes(x = factor(Scenario), y = Emissions.in.mmt)) + 
  ...
  theme(text = element_text(size=6, family = "sans"),
        ...
        legend.title = element_text(size = 4, family = "sans"),
        axis.title.y = element_text(hjust = .5)) +
  labs(x = "", y = str_wrap("US Transit Bus Fleet GHGs Over 14 Year Period (MMT)", width = 25),
       title = "        Electrifying transit buses reduces greenhouse gases") +
  scale_x_discrete(labels = scen_labels) +
  scale_y_continuous(breaks = seq(0,70,10),
                     limits = c(0,70))
ghgs

ggsave("Graphs/toc_graph.png",
       plot = ghgs,
       dpi = 700,
       width = 8.25,
       height = 4.45,
...
CO <- ggplot(data = CO_plot, aes(x = factor(Scenario), y=Emissions.in.mmt, fill=Emission.Type)) +
  ...
  geom_errorbar(aes(ymax = Emissions.in.mmt.high, ymin=Emissions.in.mmt.low, width = 0.3),
                position = position_dodge(0.8)) +
  theme_classic() +
...
emissions <- ggplot(data = emiss_plot, aes(x = factor(Scenario), y=Emissions.in.mmt, fill=Emission.Type)) +
  ...
  geom_errorbar(aes(ymax = Emissions.in.mmt.high, ymin=Emissions.in.mmt.low, width = 0.5),
                position = position_dodge(0.8)) +
  theme_classic() +
  theme(text = element_text(size=10, family = "sans"),
        axis.text = element_text(size = 9, family = "sans"),
...
final_costs[final_costs$Scenario == 1, "Min"] <- sum(scen_1_cost[scen_1_cost$Variation == "low", "Total.Fleet.Vehicles"] * 
                                                       ...
                                                       bean_pred[bean_pred$year == 2022, "Optimistic"], na.rm = TRUE) + 
...
final_costs[final_costs$Scenario == 1, "Mean"] <- sum(scen_1_cost[scen_1_cost$Variation == "avg", "Total.Fleet.Vehicles"] * 
                                                        ...
                                                        bean_pred[bean_pred$year == 2022, "Medium"], na.rm = TRUE) + 
...
final_costs[final_costs$Scenario == 1, "Max"] <- sum(scen_1_cost[scen_1_cost$Variation == "high", "Total.Fleet.Vehicles"] * 
                                                       ...
                                                       bean_pred[bean_pred$year == 2022, "Pessimistic"], na.rm = TRUE) + 
  sum(scen_1_cost[scen_1_cost$Variation == "high", "Total.Fleet.Vehicles"] * 
        beb_frame, na.rm = TRUE)

scen_2_cost <- left_join(scen_2_cost, bean_pred, by=c("Year.Start" = "year"))
...
scen_2_cost[scen_2_cost$Variation == "low", "Cost"] <- scen_2_cost[scen_2_cost$Variation == "low", "Total.Fleet.Vehicles"] * 
  ...
  scen_2_cost[scen_2_cost$Variation == "low", "Total.Fleet.Vehicles"] * 
  beb_frame

final_costs[final_costs$Scenario == 2, "Min"] <- sum(scen_2_cost[scen_2_cost$Variation == "low", "Cost"], na.rm = TRUE)
...
scen_2_cost[scen_2_cost$Variation == "avg", "Cost"] <- scen_2_cost[scen_2_cost$Variation == "avg", "Total.Fleet.Vehicles"] * 
  ...
  scen_2_cost[scen_2_cost$Variation == "avg", "Total.Fleet.Vehicles"] * 
  beb_frame

final_costs[final_costs$Scenario == 2, "Mean"] <- sum(scen_2_cost[scen_2_cost$Variation == "avg", "Cost"], na.rm = TRUE)
...
scen_2_cost[scen_2_cost$Variation == "high", "Cost"] <- scen_2_cost[scen_2_cost$Variation == "high", "Total.Fleet.Vehicles"] * 
  ...
  scen_2_cost[scen_2_cost$Variation == "high", "Total.Fleet.Vehicles"] * 
  beb_frame

final_costs[final_costs$Scenario == 2, "Max"] <- sum(scen_2_cost[scen_2_cost$Variation == "high", "Cost"], na.rm = TRUE)

s2_sched <- scen_2_cost %>% group_by(Year.Start,
                                     Variation) %>% summarise(Cost = sum(round(Cost/1000000000, 3)))

scen_3_cost <- left_join(scen_3_cost, bean_pred, by=c("Year.Start" = "year"))

scen_3_cost[scen_3_cost$Variation == "avg", "Cost"] <- scen_3_cost[scen_3_cost$Variation == "avg", "Total.Fleet.Vehicles"] * 
  scen_3_cost[scen_3_cost$Variation == "avg", "batt_size"] * 
  scen_3_cost[scen_3_cost$Variation == "avg", "Medium"] + 
  scen_3_cost[scen_3_cost$Variation == "avg", "Total.Fleet.Vehicles"] * beb_frame

final_costs[final_costs$Scenario == 3, "Mean"] <- sum(scen_3_cost[scen_3_cost$Variation == "avg", "Cost"], na.rm = TRUE)

scen_3_cost[scen_3_cost$Variation == "low", "Cost"] <- scen_3_cost[scen_3_cost$Variation == "low", "Total.Fleet.Vehicles"] * 
  scen_3_cost[scen_3_cost$Variation == "low", "batt_size"] * 
  scen_3_cost[scen_3_cost$Variation == "low", "Optimistic"] + 
  scen_3_cost[scen_3_cost$Variation == "low", "Total.Fleet.Vehicles"] * beb_frame

final_costs[final_costs$Scenario == 3, "Min"] <- sum(scen_3_cost[scen_3_cost$Variation == "low", "Cost"], na.rm = TRUE)

scen_3_cost[scen_3_cost$Variation == "high", "Cost"] <- scen_3_cost[scen_3_cost$Variation == "high", "Total.Fleet.Vehicles"] * 
  scen_3_cost[scen_3_cost$Variation == "high", "batt_size"] * 
  scen_3_cost[scen_3_cost$Variation == "high", "Pessimistic"] + 
  scen_3_cost[scen_3_cost$Variation == "high", "Total.Fleet.Vehicles"] * beb_frame

final_costs[final_costs$Scenario == 3, "Max"] <- sum(scen_3_cost[scen_3_cost$Variation == "high", "Cost"], na.rm = TRUE)

s3_sched <- scen_3_cost %>% group_by(Year.Start,
                                     Variation) %>% summarise(Cost = sum(round(Cost/1000000000, 3)))

scen_4_cost <- left_join(scen_4_cost, bean_pred, by=c("Year.Start" = "year"))

scen_4_cost[scen_4_cost$Variation == "avg", "Cost"] <- scen_4_cost[scen_4_cost$Variation == "avg", "Total.Fleet.Vehicles"] * 
  scen_4_cost[scen_4_cost$Variation == "avg", "batt_size"] * 
  scen_4_cost[scen_4_cost$Variation == "avg", "Medium"] + 
  scen_4_cost[scen_4_cost$Variation == "avg", "Total.Fleet.Vehicles"] * beb_frame

final_costs[final_costs$Scenario == 4, "Mean"] <- sum(scen_4_cost[scen_4_cost$Variation == "avg", "Cost"], na.rm = TRUE)

scen_4_cost[scen_4_cost$Variation == "low", "Cost"] <- scen_4_cost[scen_4_cost$Variation == "low", "Total.Fleet.Vehicles"] * 
  scen_4_cost[scen_4_cost$Variation == "low", "batt_size"] * 
  scen_4_cost[scen_4_cost$Variation == "low", "Optimistic"] + 
  scen_4_cost[scen_4_cost$Variation == "low", "Total.Fleet.Vehicles"] * beb_frame

final_costs[final_costs$Scenario == 4, "Min"] <- sum(scen_4_cost[scen_4_cost$Variation == "low", "Cost"], na.rm = TRUE)

scen_4_cost[scen_4_cost$Variation == "high", "Cost"] <- scen_4_cost[scen_4_cost$Variation == "high", "Total.Fleet.Vehicles"] * 
  scen_4_cost[scen_4_cost$Variation == "high", "batt_size"] * 
  scen_4_cost[scen_4_cost$Variation == "high", "Pessimistic"] + 
  scen_4_cost[scen_4_cost$Variation == "high", "Total.Fleet.Vehicles"] * beb_frame

final_costs[final_costs$Scenario == 4, "Max"] <- sum(scen_4_cost[scen_4_cost$Variation == "high", "Cost"], na.rm = TRUE)

s4_sched <- scen_4_cost %>% group_by(Year.Start,
                                     Variation) %>% summarise(Cost = sum(round(Cost/1000000000, 3)))
...
s3_sched$Scenario <- 3
s4_sched$Scenario <- 4
temp <- rbind.data.frame(s2_sched, s3_sched)
schedule <- rbind.data.frame(temp, s4_sched)
...
ny_data <- filter(scen_1, Agency.Name.x == "MTA New York City Transit" & Total.Miles.on.Active.Vehicles.During.Period > 0)
ny_data$vmt.per.veh <- ny_data$Total.Miles.on.Active.Vehicles.During.Period/ny_data$Active.Fleet.Vehicles
ny.vmt <- ny_data$vmt.per.veh %>% max(na.rm = TRUE)
...
lcod <- data.frame(Fuel.Type = c("Diesel", "Compressed Natural Gas", "Electric"),
                   Discount.Rate = 0.05,
                   Discount.Rate.pes = 0.07,
                   Discount.Rate.opt = 0.03,
                   Lifetime = 12,
                   Capital.Costs.dollar = c(557000, 649000, beb.mean),
                   Capital.Costs.dollar.pes = c(728000, 897000, beb.max),
                   Infrastructure.dollar = c(0, 62000, 56000),
                   Infrastructure.dollar.pes = c(0, 68200, 68000),
                   Maintenance.dollar..mi = c(1.83, 1.83, 1.58),
                   Maintenance.dollar..mi.pes = c(1.83, 1.83, 1.58),
                   Maintenance.dollar..mi.opt = c(0.4, 0.4, 0.19),
                   all.VMT = c(all.vmt),
                   pes.VMT = c(eb.vmt),
                   opt.VMT = c(ny.vmt),
                   Fuel.Price.dollar..dge = c(4.58, 3.68, elec.avg),
                   Fuel.Price.dollar..dge.pes = c(5.75, 5.36, elec.max),
                   Fuel.Price.dollar..dge.opt = c(4.03, 2.88, elec.min),
                   MPGDE = c(4.7, 5.2, 22.0))

# Calculate how electric fuel price will be affected by plug efficiency
lcod$Fuel.Price.dollar..dge[3] <- lcod$Fuel.Price.dollar..dge[3] / plug_eff
lcod$Fuel.Price.dollar..dge.pes[3] <- lcod$Fuel.Price.dollar..dge.pes[3] / plug_eff
lcod$Fuel.Price.dollar..dge.opt[3] <- lcod$Fuel.Price.dollar..dge.opt[3] / plug_eff
...
lcod$Capital <- (lcod$CRF*lcod$Capital.Costs.dollar)/(lcod$all.VMT)
lcod$Infrastructure <- (lcod$CRF*lcod$Infrastructure.dollar)/(lcod$all.VMT)
...
# Calculate +/- 10% for infrastructure costs besides BEB
...
lcod$Capital.pes <- (lcod$CRF.pes * lcod$Capital.Costs.dollar.pes)/(lcod$pes.VMT)
lcod$Infrastructure.pes <- (lcod$CRF.pes * lcod$Infrastructure.dollar.pes)/lcod$pes.VMT
...
lcod$final.pes <- (lcod$CRF.pes*(lcod$Capital.Costs.dollar.pes + lcod$Infrastructure.dollar.pes))/
  (lcod$pes.VMT) + lcod$Maintenance.dollar..mi.pes + lcod$Fuel.Price.dollar..dge.pes/lcod$MPGDE

lcod$CRF.opt <- (lcod$Discount.Rate.opt*(1+lcod$Discount.Rate.opt)^lcod$Lifetime)/
  ((1+lcod$Discount.Rate.opt)^lcod$Lifetime - 1)
lcod$Capital.opt <- (lcod$CRF.opt * lcod$Capital.Costs.dollar)/(lcod$opt.VMT)
lcod$Infrastructure.opt <- (lcod$CRF.opt * lcod$Infrastructure.dollar.opt)/lcod$opt.VMT
...
lcod$final.opt <- (lcod$CRF.opt*(lcod$Capital.Costs.dollar + lcod$Infrastructure.dollar.opt))/
  (lcod$opt.VMT) + lcod$Maintenance.dollar..mi.opt + lcod$Fuel.Price.dollar..dge.opt/lcod$MPGDE


lcod_melt <- lcod[, c(1, 21:24)]
...
lcod_save <- ggplot(data = lcod_plot, aes(fill = Costs, y=value, x=Fuel.Type)) + 
  ...
  annotate(geom = "label", x = 1, y = lcod$final[1] + 1, label = sprintf("%.2f", lcod$final[1]), family = "sans", size = 5) +
...
# BEB to diesel
parity(lcod = lcod[lcod$Fuel.Type == "Diesel", "final"],
...
lcod_func <- function(discount.rate, 
                      ...
                      fuel.price,
                      mpdge){
  lcod <- (((discount.rate * (1 + discount.rate)^lifetime) / ((1 + discount.rate)^lifetime - 1)) * (capital + infrastructure)) / vmt + maintenance + fuel.price/mpdge
  return(lcod)
}
...
scc <- function(savings,
                ghg.mitigated,
                lifetime,
                no.of.buses,
                vmt){
  scc = savings * (lifetime * no.of.buses * vmt) / ghg.mitigated
  return(scc)
}
...
# GHGs mitigated in all scenario on average
...
(ghg_plot[ghg_plot$Scenario == 0, "Emissions.in.mmt"] - ghg_plot[ghg_plot$Scenario == 2, "Emissions.in.mmt"])/ghg_plot[ghg_plot$Scenario == 0, "Emissions.in.mmt"]
...
(ghg_plot[ghg_plot$Scenario == 0, "Emissions.in.mmt"] - ghg_plot[ghg_plot$Scenario == 4, "Emissions.in.mmt"])/ghg_plot[ghg_plot$Scenario == 0, "Emissions.in.mmt"]

# GHGs mitigated in all scenarios worst case
(ghg_plot[ghg_plot$Scenario == 0, "Emissions.in.mmt.high"] - ghg_plot[ghg_plot$Scenario == 1, "Emissions.in.mmt.high"])/ghg_plot[ghg_plot$Scenario == 0, "Emissions.in.mmt.high"]
(ghg_plot[ghg_plot$Scenario == 0, "Emissions.in.mmt.high"] - ghg_plot[ghg_plot$Scenario == 2, "Emissions.in.mmt.high"])/ghg_plot[ghg_plot$Scenario == 0, "Emissions.in.mmt.high"]
(ghg_plot[ghg_plot$Scenario == 0, "Emissions.in.mmt.high"] - ghg_plot[ghg_plot$Scenario == 3, "Emissions.in.mmt.high"])/ghg_plot[ghg_plot$Scenario == 0, "Emissions.in.mmt.high"]
(ghg_plot[ghg_plot$Scenario == 0, "Emissions.in.mmt.high"] - ghg_plot[ghg_plot$Scenario == 4, "Emissions.in.mmt.high"])/ghg_plot[ghg_plot$Scenario == 0, "Emissions.in.mmt.high"]
# lbs of CO2 per passenger mile
ghg_plot[ghg_plot$Scenario == 0, "Emissions.in.mt"] * 2204.62 / (bus_mileage$Passenger.Miles %>% sum())
